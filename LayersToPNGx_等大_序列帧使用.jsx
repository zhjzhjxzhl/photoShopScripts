...
function run () {
	...
	for (var skinName in skins) {
		...
		for (var i = skinLayersCount - 1; i >= 0; i--) {
			...
			layer.visible = true;
			//if (!layer.isBackgroundLayer && !noResize) activeDocument.trim(TrimType.TRANSPARENT, false, true, true, false);
			x -= activeDocument.width.as("px") * pngScale;
			y -= activeDocument.height.as("px") * pngScale;
			//if (!layer.isBackgroundLayer && !noResize) activeDocument.trim(TrimType.TRANSPARENT, true, false, false, true);
			...
			if (writePngs) {
				...
				//if (padding > 0 && !noResize) activeDocument.resizeCanvas(width, height, AnchorPosition.MIDDLECENTER);
				if(noResize)
				{
					...
					var cx = (layer.bounds[2].as("px")+layer.bounds[0].as("px"))/2;
					var cy = (layer.bounds[3].as("px")+layer.bounds[1].as("px"))/2;
					var width1 = (Math.abs(cx-ww/2)+lw/2)*2;
					var height1 = (Math.abs(cy-hh/2)+lh/2)*2;
					activeDocument.resizeCanvas(width1, height1, AnchorPosition.MIDDLECENTER);
				}
...
function showDialog () {
	...
	var checkboxGroup = dialog.add("group");
		var group = checkboxGroup.add("group");
			...
			var noResizeCheckbox = group.add("checkbox",undefined,"no resize");
			noResizeCheckbox.value = noResize;
	...
	function updateSettings () {
		...
		noResize = noResizeCheckbox.value;
...
function pngSaveForWeb (file) {
	...
	desc2.putBoolean(app.charIDToTypeID("Mtt "), false);
	...
	desc2.putPath(app.charIDToTypeID("In "), new File(file));
